...
async function resolveFilmArtifacts(projectDir) {
  const summaryPath = path.join(projectDir, FILM_PROJECT_SUMMARY_FILE);
  const summary = (await pathExists(summaryPath))
    ? {
        canonicalFileName: FILM_PROJECT_SUMMARY_FILE,
        actualFileName: FILM_PROJECT_SUMMARY_FILE,
  ...
  for (const step of FILM_PROGRESS_STEPS) {
    let matchedFileName = "";
    let matchedMtimeMs = 0;
    for (const candidate of stepFileCandidates(step)) {
      const candidatePath = path.join(projectDir, candidate);
      if (!(await pathExists(candidatePath))) {
        continue;
      }
      matchedFileName = candidate;
      matchedMtimeMs = (await stat(candidatePath)).mtimeMs;
      break;
    }
    steps.push({
      ...step,
      present: Boolean(matchedFileName),
      actualFileName: matchedFileName,
      mtimeMs: matchedMtimeMs,
    });
  }
