...
For long-form opening videos, the storyboard must also prove that the runtime is earned.
Do not stretch one short-form motif across the whole film when the brief calls for a 60-120 second opening with real progression.
...
- dramatic purpose
- visual objective
- visual system or content layer
...
11. If the brief is long-form and opening-led, prove progression through section changes, content escalation, or visual-system rotation rather than repeating one reveal mechanic.
...
- if `Opening Video` is `60` seconds or longer, default to `3-5` storyboard sections with distinct visual jobs
- let one brand DNA hold the film together, but allow different sections to shift the visual system, information density, or scale
- treat concrete Step 1 reference elements such as `particles`, `big title`, or `logo reveal` as section tools, not automatic full-film loops
- if the storyboard uses people, factory, device, system UI, or global-capability material, stage them as high-impact proof beats or stylized tableaux rather than exhaustive enterprise-promo coverage
- keep the shot design selective, escalatory, and reveal-led so the film still plays like an opening piece, not a corporate profile reel
- if multiple sections reuse the same motif, each reuse must escalate or transform its role
